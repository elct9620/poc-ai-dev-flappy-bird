...
- 比較小鳥的右側邊緣（x 位置 + 小鳥寬度 × 縮放比例）與管道的右側邊緣（x 位置 + 管道寬度 × 縮放比例）
- 當小鳥的右側邊緣超過管道的右側邊緣時，視為成功通過
- 小鳥精靈尺寸為 34×24 像素（來自 `src/assets/gameObjects/yellowbird-*.png`）
- 管道寬度為 52 像素（來自精靈圖片尺寸）
- 所有尺寸都會根據響應式縮放比例調整，確保在不同螢幕尺寸下判定一致
- 每對管道的「已通過」旗標（passed flag）確保只觸發一次計分
...
4. **計分或失敗**：
   - **成功通過管道**：
     - 障礙物系統透過邊緣比較偵測小鳥通過管道
     - 比較小鳥右側邊緣（x + 34×縮放）與管道右側邊緣（x + 52×縮放）
     - 當小鳥邊緣超過管道邊緣時，標記管道為「已通過」
