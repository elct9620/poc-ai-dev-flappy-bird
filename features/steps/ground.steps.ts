...
// Then steps
Then(
  "the ground {string} should exist in the game state",
  (world: GameWorld, id: string) => {
    const state = world.getState();
    expect(state.entities[id]).toBeDefined();
    expect(state.entities[id].type).toBe("ground");
  },
);

Then(
  "the ground {string} should have type {string}",
...
Then(
  "the ground {string} should not exist in the game state",
  (world: GameWorld, id: string) => {
    const state = world.getState();
